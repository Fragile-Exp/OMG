...
public class DocumentDaoImpl {
	...
	RowMapper rowMapper = new RowMapper<DocumentVO>() {
		...
		public DocumentVO mapRow(ResultSet rs, int rowNum) throws SQLException {
		    ...
		    outVO.setDocumentId(rs.getString("documentId"));     //Î¨∏ÏÑúÍ≥†Ïú†Î≤àÌò∏  
		    outVO.setEmployeeId(rs.getString("employeeId"));     //?Ç¨Î≤?    
		    outVO.setKind(rs.getInt("kind"));                      //Î¨∏ÏÑúÏ¢ÖÎ•ò  
		    outVO.setTitle(rs.getString("title"));                 //Î¨∏ÏÑú?†úÎ™?  
		    outVO.setdDay(rs.getString("dDay"));                 //Ï≤òÎ¶¨Í∏∞Í∞Ñ  
		    outVO.setDocumentCont(rs.getString("documentCont")); //Î¨∏ÏÑú?Ç¥?ö©  
		    outVO.setDocumentSet(rs.getInt("documentSet"));      //Î¨∏ÏÑú?ÉÅ?Éú  
		    outVO.setOkUser(rs.getString("okUser"));             //Í≤∞Ïû¨?ûê?Ç¨Î≤? 
